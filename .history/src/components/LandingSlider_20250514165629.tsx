        ...
        <div key={job.id} className="flex flex-col gap-2">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm font-semibold italic text-zinc-600 dark:text-zinc-400">
  ...
  return (
    <div className="flex flex-col gap-8">
      {projects.map((project) => (
        <div
          key={project.name}
          className="flex items-start gap-4 rounded-lg border border-zinc-100 p-4 dark:border-zinc-700/60"
        >
...
export default function LandingSlider({ articles }: { articles: ArticleWithSlug[] }) {
  ...
  return (
    <div className="w-full">
      <div className="flex space-x-1 mb-8 p-1 border rounded-lg border-zinc-200 dark:dark:border-zinc-700/60 overflow-none">
        {tabs.map((tab) => (
          <button
            key={tab.id}
