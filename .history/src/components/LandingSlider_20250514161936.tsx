...
export default function LandingSlider({ articles }: { articles: ArticleWithSlug[] }) {
  ...
  return (
    <div className="w-full">
      <div className="flex space-x-1 mb-8 p-1 border rounded-lg border-zinc-200 dark:border-gray-500 overflow-none">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={clsx(
              'px-4 py-2 rounded transition-all duration-200 font-medium text-sm ',
              activeTab === tab.id
                ? ' flex-1 bg-zinc-50 font-medium text-zinc-900 hover:bg-zinc-100 active:bg-zinc-100 active:text-zinc-900/60 dark:bg-zinc-800/50 dark:text-zinc-300 dark:hover:bg-zinc-800 dark:hover:text-zinc-50 dark:active:bg-zinc-800/50 dark:active:text-zinc-50/70'
